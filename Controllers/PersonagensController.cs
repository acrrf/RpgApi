    ...
    public class PersonagensController:ControllerBase
    {
        ...
        public async Task<IActionResult> GetSingle(int id)
        {
            try
            {
                ...
                //Desafio 4 - Ok
                .Include(us=> us.Usuario)//inclui na propriedade Usuário do objeto p
        ...
        public async Task<IActionResult> RestaurarPontosVidaAsync(Personagem p)
        {
            try
            {
                ...
                Personagem? pEncontrado = await _dataContext.Personagens.FirstOrDefaultAsync(pBusca => pBusca.Id == p.Id);
                ...
                if (atualizou) linhasAfetadas = await _dataContext.SaveChangesAsync();
                return Ok(linhasAfetadas);
            }
        ...
        //Metodo para alteração da foto
        [HttpPut("AtualizarFoto")]
        public async Task<IActionResult> AtualizarFotoAsync(Personagem p)
        {
            try
            {
                Personagem? personagem = await _dataContext.Personagens.FirstOrDefaultAsync(x => x.Id == p.Id);
                personagem.FotoPersonagem = p.FotoPersonagem;
                var attach = _dataContext.Attach(personagem);
        ...
        public async Task<IActionResult> GetByUserAsync(int userId)
        {
            try
            {
                List<Personagem> lista = await _dataContext.Personagens.Where(u => u.Usuario.Id == userId).ToListAsync();
        ...
        public async Task<IActionResult> GetByPerfilAsync(int userId)
        {
            try
            {
                Usuario? usuario = await _dataContext.Usuarios.FirstOrDefaultAsync(x => x.Id == userId);
                List<Personagem> lista = new List<Personagem>();
                if (usuario.Perfil == "Admin") 
                    lista = await _dataContext.Personagens.ToListAsync();
                else 
                    lista = await _dataContext.Personagens.Where(p=> p.Usuario.Id == userId).ToListAsync();
